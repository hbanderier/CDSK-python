...
class Lorenz84(DiffDynSyst):
    ...
    class TimeForcing:  ##{{{
        ...
        def cyclic(t):
            ...
            t    : float
                    Time
            """
            return 9.5 + 2.0 * np.sin(t * 2.0 * np.pi / 73.0)
    ...
    def __init__(self, a=0.25, b=4.0, G=1.0, F=None, size=1):
        """
        Parameters
        ----------
        a    : float
        ...
        if callable(F):
            ...
            if F == "cyclic":
                ...
                self.F = lambda t: self.TimeForcing.cyclic(t) + self.TimeForcing.linear(
                    t
                )
        else:
