    ...
    def _solver(self, X0, time):
        if self._edo_solver == "RK4":
            ...
            for i in range(sTime):
                ...
                k2 = self._equation(solution[i, :] + h * k1 / 2.0, time[i] + h / 2.0)
                k3 = self._equation(solution[i, :] + h * k2 / 2.0, time[i] + h / 2.0)
                k4 = self._equation(solution[i, :] + h * k3, time[i + 1])
                solution[i + 1, :] = (
                    solution[i, :] + h * (k1 + 2 * k2 + 2 * k3 + k4) / 6.0
                )
            return solution
            ...
            for i in range(sTime):
                h = time[i + 1] - time[i]
                solution[i + 1, :] = solution[i, :] + h * self._equation(
                    solution[i, :], time[i]
                )
            return solution
        else:
