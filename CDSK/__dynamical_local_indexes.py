...
def compute_theta(X, ql=0.98, theta_fit="sueveges", n_jobs=os.cpu_count(), **kwargs):
    ...
    theta = np.zeros((n_sample, n_traj, n_traj)) + np.nan
    for i, j in itt.combinations_with_replacement(range(n_traj), 2):
        q = np.quantile(dist[:, :, i, j], ql, 1)
        where = dist[:, :, i, j] > q.reshape(-1, 1)
        for k in range(n_sample):
            idx = np.argwhere(where[k, :]).squeeze()
            if theta_fit == "ferro":
...
def dynamical_local_indexes(
    ...
    **kwargs
    ...
    total_iter = comb(n_traj, 2, repetition=True)
    ...
    for i, j in itt.combinations_with_replacement(range(n_traj), 2):
        for k in trange(n_sampleX):
            idx = np.atleast_1d(np.argwhere(where[k, :, i, j]).squeeze())
            if theta_fit == "ferro":
                theta[k, i, j] = _theta_ferro(idx)
            else:
                theta[k, i, j] = _theta_sueveges(idx, ql)
